...
const MonitorForm: React.FC<MonitorFormProps> = ({
  ...
  const [value, setValue] = useState(initialValues?.interval || 5);
  const options = [1, 5, 30, 60, 720, 1440];
  const labels = ["1m", "5m", "30m", "1h", "12h", "24h"];
  const [selectedMonitor, setSelectedMonitor] = useState(
    monitorTypes.find((type) => type.value === initialValues?.monitorType) ||
      monitorTypes[0]
  );
  ...
  return (
    <div className="fixed top-0 left-0 flex justify-center items-center w-full h-full px-24 z-[1000]">
      <div
        onClick={onCancel}
    ...
    ${isDropdownOpen ? "max-h-[800px] opacity-100" : "max-h-0 opacity-0"}`}
          ...
          >
            {initialValues ? "Update Monitor" : "Create Monitor"}
          </button>
        </div>
      </form>
    </div>
